...
*/
CREATE TABLE graph(source integer, 
		   target integer, 
		   weight integer);
		   
CREATE TABLE visited_graph(source integer, 
			   target integer, 
			   weight integer,
			   visited boolean);
			   
CREATE TABLE minimum_spanning_tree(source integer, 
				   target integer, 
				   weight integer);
...
INSERT INTO visited_graph 
     SELECT g.source, g.target, g.weight, false 
       FROM graph g;
...
CREATE OR REPLACE FUNCTION create_min_spanning_tree()
 ...
 $$
 	...
 	BEGIN
 		...
 		--Get count of nodes;
		SELECT INTO unvisited_nodes count(1) 
 		...
 		min_valid_weight AS ( SELECT min(vg.weight) AS weight 
 		...
 		valid_edge AS ( SELECT vg.source AS src, vg.target AS trg 
 				  FROM visited_graph vg 
			  NATURAL JOIN min_valid_weight mvw
 			  	 LIMIT 1 )
 		...
 		WHILE unvisited_nodes != 0
 		 LOOP
 		 	...
 		 	valid_nodes AS ( SELECT un.source 
					   FROM unvisited_nodes un 
 		 		  EXCEPT SELECT vn.source 
					   FROM visited_nodes vn 
				         ),
 		 	--From all valid edges, get the minimum weight 
 		 	min_valid_weight AS ( SELECT min(vg.weight) AS weight
 		 				FROM visited_graph vg 
					NATURAL JOIN valid_nodes vn 
					     ),
